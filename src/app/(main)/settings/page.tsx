...
export default async function SettingsPage() {
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
      ...
      <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
        ...
        <div className="space-y-3">
          <div className="space-y-1">
            ...
            <p className="text-sm font-medium text-foreground">{session.user.name ?? "Sin nombre"}</p>
          </div>
          <div className="space-y-1">
      ...
      {/* Learning config */}
      <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
        <h2 className="font-semibold text-foreground text-sm">Configuración de aprendizaje</h2>
        ...
        <div className="space-y-3">
          {session.user.currentLevel && (
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm font-medium text-foreground">CEFR {session.user.currentLevel}</p>
              </div>
              <Badge variant={session.user.currentLevel as CEFRLevel}>{session.user.currentLevel}</Badge>
            </div>
          )}
          ...
          {session.user.currentObjective && (
            <div>
              ...
              <p className="text-sm font-medium text-foreground capitalize">{session.user.currentObjective}</p>
            </div>
          )}
        </div>
